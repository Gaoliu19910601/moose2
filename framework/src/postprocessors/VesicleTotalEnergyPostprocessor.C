...
{
  const VesicleTotalEnergyPostprocessor & pps = static_cast<const VesicleTotalEnergyPostprocessor &>(y);
...
VesicleTotalEnergyPostprocessor::computeQpIntegral()
{
  ...
  Real value = 0.5 * pow(_second_u[_qp].tr() + _grad_u[_qp](0)/rz_coord - 1.0/(_epsilon * _epsilon) * (_u[_qp] * _u[_qp] - 1) * _u[_qp] - _C, 2.0);
  return value;
}

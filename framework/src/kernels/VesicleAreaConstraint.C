...
VesicleAreaConstraint::computeQpResidual()
{
  ...
  return _test[_i][_qp] * _shp_integral * _alpha_a * (-3.0/std::sqrt(2.0) * _epsilon * (_second_u[_qp].tr() + _grad_u[_qp](0)/rz_coord));
}
...
VesicleAreaConstraint::computeQpJacobian()
{
  ...
  return _test[_i][_qp] * _shp_jacobian[_var_dofs[_j]] * _alpha_a * (-3.0/std::sqrt(2.0) * _epsilon * (_second_u[_qp].tr() + _grad_u[_qp](0)/rz_coord));
}
...
VesicleAreaConstraint::computeQpNonlocalJacobian(dof_id_type dof_index)
{
 ...
 return _test[_i][_qp] * _shp_jacobian[dof_index] * _alpha_a * (-3.0/std::sqrt(2.0) * _epsilon * (_second_u[_qp].tr() + _grad_u[_qp](0)/rz_coord));
}

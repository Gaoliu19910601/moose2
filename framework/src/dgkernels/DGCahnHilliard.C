...
DGCahnHilliard::computeQpResidual(Moose::DGResidualType type)
{
  ...
  {
  case Moose::Element:
    r += -_kappa[_qp] * 0.5 * (_second_u[_qp].tr() + _second_u_neighbor[_qp].tr()) * (_grad_test[_i][_qp] * _normals[_qp]);
    r += -_kappa[_qp] * (_grad_u[_qp] * _normals[_qp] - _grad_u_neighbor[_qp] * _normals[_qp]) * 0.5 * (_second_test[_i][_qp].tr()); 
    r += _eta / h_elem * (_grad_u[_qp] * _normals[_qp] - _grad_u_neighbor[_qp] * _normals[_qp]) * (_grad_test[_i][_qp] * _normals[_qp]);
    break;

  case Moose::Neighbor:
    r += -_kappa[_qp] * 0.5 * (_second_u[_qp].tr() + _second_u_neighbor[_qp].tr()) * (-_grad_test_neighbor[_i][_qp] * _normals[_qp]);
    r += -_kappa[_qp] * (_grad_u[_qp] * _normals[_qp] - _grad_u_neighbor[_qp] * _normals[_qp]) * 0.5 * (_second_test_neighbor[_i][_qp].tr());
...
DGCahnHilliard::computeQpJacobian(Moose::DGJacobianType type)
{
  ...
  case Moose::ElementNeighbor:
    r += -_kappa[_qp] * 0.5 * (_second_phi_neighbor[_j][_qp].tr() * _grad_test[_i][_qp] * _normals[_qp]);
    r += -_kappa[_qp] * (-_grad_phi_neighbor[_j][_qp]) * _normals[_qp] * 0.5 * _second_test[_i][_qp].tr();
    r += _eta / h_elem * (-_grad_phi_neighbor[_j][_qp] * _normals[_qp]) * (_grad_test[_i][_qp] * _normals[_qp]);
    break;

  case Moose::NeighborElement:
    r += -_kappa[_qp] * 0.5 * (_second_phi[_j][_qp].tr() * (-_grad_test_neighbor[_i][_qp]) * _normals[_qp]);
    r += -_kappa[_qp] *_grad_phi[_j][_qp] * _normals[_qp] * 0.5 * _second_test_neighbor[_i][_qp].tr();
    r += _eta / h_elem * (_grad_phi[_j][_qp] * _normals[_qp]) * (-_grad_test_neighbor[_i][_qp] * _normals[_qp]);
    break;

  case Moose::NeighborNeighbor:
    r += -_kappa[_qp] * 0.5 * (_second_phi_neighbor[_j][_qp].tr() * (-_grad_test_neighbor[_i][_qp]) * _normals[_qp]);
    r += -_kappa[_qp] * (-_grad_phi_neighbor[_j][_qp]) * _normals[_qp] * 0.5 * _second_test_neighbor[_i][_qp].tr();
